...
val androidStoreFile = providers.gradleProperty("KLAWTY_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("KLAWTY_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("KLAWTY_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("KLAWTY_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set KLAWTY_ANDROID_STORE_FILE, " +
            "KLAWTY_ANDROID_STORE_PASSWORD, KLAWTY_ANDROID_KEY_ALIAS, and " +
            "KLAWTY_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/KlawtyKit/Sources/KlawtyKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.klawty.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "klawty-$versionName-$buildType.apk"
