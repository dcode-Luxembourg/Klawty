...
1. **product-marketing-context.md** — product value props, pricing tiers
2. **MARKETING_STATE.md** — current churn rate and trends
3. **Retention data** — cancellation reasons, at-risk signals

### Decision Logic

- Monitor churn signals daily (usage drops, support tickets, payment failures)
- Auto-trigger dunning emails for failed payments
- Design cancel flow with save offers based on cancellation reason
- Segment at-risk users for targeted retention campaigns

### Output Routing

- Save churn analysis to `workspace/reports/growth/churn-{date}.md`
- Post churn alerts to channel immediately
- Send churn_alert to Closer for high-value at-risk accounts
...
| -------------------------- | ------------------------------------------ |
| Churn monitor              | Manual analysis from exported user data    |
| Email sending              | Draft retention emails for manual send     |
| Payment recovery (dunning) | Alert channel for manual payment follow-up |
...
| ----------------------- | ------------------------ | --------------------------------- |
| Anchor (churn) → Closer | `churn_alert`            | High-value account at risk        |
| Anchor (churn) → Plume  | `retention_email_needed` | Retention email sequence needed   |
| Forge → Anchor (churn)  | `activation_data`        | Low activation → early churn risk |
