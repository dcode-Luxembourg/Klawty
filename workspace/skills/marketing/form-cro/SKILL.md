...
1. **product-marketing-context.md** — lead qualification criteria, conversion goals
2. **CRO_BASELINE.md** — form completion rates
3. **Analytics** — form abandonment data if available

### Decision Logic

- Audit all non-signup forms (contact, demo request, lead capture)
- Prioritize by form traffic × completion rate gap
- Focus: field reduction, multi-step conversion, trust signals

### Output Routing

- Save audit to `workspace/reports/cro/form-audit-{date}.md`
- Post findings with specific field-by-field recommendations
...
| ---------------- | -------------------------------------------------------- |
| Analytics        | Audit form structure against best practices              |
| Heatmaps         | No field-level interaction data, use structural analysis |
...
| --------------------- | --------------------- | ------------------------------------------------- |
| Forge (form) → Plume  | `test_result`         | Form copy changes needed based on test results    |
| Forge (form) → Closer | `lead_form_optimized` | Lead form improved, review qualification criteria |
