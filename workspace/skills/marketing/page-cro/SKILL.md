...
1. **product-marketing-context.md** — product, audience, conversion action, objections
2. **CRO_BASELINE.md** — previous audit results for comparison
3. **Analytics** — conversion rates, bounce rates, funnel data
4. **Heatmaps** — if tool CONNECTED, behavioral data

### Decision Logic

- Run weekly audit on homepage + top 3 landing pages
- Compare against CRO_BASELINE.md to detect regressions
- Prioritize recommendations by expected revenue impact
- Auto-generate experiment hypotheses for top issues

### Output Routing

- Save audit to `workspace/reports/cro/page-audit-{date}.md`
- Post top 3 findings + recommendations to channel
- Send experiment proposals to channel for approval (ASSISTED) or auto-create (AUTONOMOUS)
...
| -------------------- | ----------------------------------------------------------- |
| Analytics            | Expert audit from page structure alone using CRO principles |
| Heatmaps             | Audit based on best practices, no behavioral data           |
| A/B testing platform | Produce recommendations only, no live experiments           |
| CMS read             | Fetch public pages via HTTP                                 |
...
| ------------- | ------------- | ------------------------------------------------- |
| Forge → Plume | `test_result` | A/B test winner — update page copy                |
| Forge → Scout | `cro_audit`   | Page has technical/SEO issues blocking conversion |
| Plume → Forge | `draft_ready` | New page copy ready for CRO review                |
