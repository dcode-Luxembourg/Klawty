...
1. **product-marketing-context.md** — product, pricing, ICP, buyer journey
2. **CRM data** — pipeline, lead scores, deal stages
3. **MARKETING_STATE.md** — lead flow and conversion metrics

### Decision Logic

- Define and enforce lead lifecycle (Subscriber → Lead → MQL → SQL → Opportunity → Customer)
- Score leads automatically based on fit + engagement signals
- Route leads to correct pipeline stage and owner
- Flag pipeline hygiene issues (stale deals, missing data)

### Output Routing

- Save pipeline reports to `workspace/reports/sales/pipeline-{date}.md`
- Post daily pipeline summary to channel
...
| ----------------- | ---------------------------------------------- |
| CRM               | Pipeline analysis from exported data only      |
| Lead scoring tool | Manual scoring framework based on ICP criteria |
| Analytics         | Use CRM-native reporting                       |
...
| ------------------------ | -------------------- | ------------------------------------ |
| Closer (revops) → Plume  | `lead_qualified`     | MQL needs nurture content            |
| Closer (revops) → Anchor | `customer_onboarded` | New customer — monitor retention     |
| Pulse → Closer (revops)  | `ad_lead`            | Paid lead captured — add to pipeline |
