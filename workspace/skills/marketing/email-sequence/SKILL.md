...
- Auto-detect sequence type from: task brief, existing sequences, conversion funnel stage
- Pull audience segments from product-marketing-context.md personas
- Default timing: welcome (Day 0,1,3,7), nurture (weekly), re-engagement (Day 30,37,44)

### Output Routing

- Email platform CONNECTED: format as platform-native automation config
- Email platform UNCONFIGURED: save as markdown in `workspace/content/emails/sequences/`
- Post sequence overview + first email preview to channel
...
| ---------------- | ----------------------------------------------------------- |
| Email platform   | Draft sequences as markdown templates with timing notes     |
| Analytics        | Use default benchmarks for open rates and timing            |
| CRM / segments   | Use generic segments (all users, new users, inactive users) |
...
| ---------------------- | ------------------------ | ------------------------------------------------------- |
| Plume (email) → Forge  | `email_lp_ready`         | Email links to landing page that needs CRO review       |
| Plume (email) → Anchor | `nurture_sequence_ready` | Nurture sequence ready, coordinate with retention flows |
