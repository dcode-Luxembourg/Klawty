/** OpenClaw Runtime — Memory Distiller */
...
// Optional: memory service for Qdrant/vector integration
...
async function distilAgent(agentName) {
  ...
  // Get recent vector facts for this agent (if memory service is available)
  let semanticFacts = "";
  if (_memoryService && typeof _memoryService.recall === "function") {
    try {
      const facts = await _memoryService.recall("", { agent: agentName, limit: 20 });
      if (facts?.length > 0) {
        semanticFacts = facts.map((f) => `- ${(f.content || "").slice(0, 150)}`).join("\n");
      }
    } catch {
      /* non-blocking — vector store may be down */
    }
  }
...
- If nothing new, return empty arrays`;
  ...
  try {
    ...
    // Consolidate near-duplicate vectors for this agent (if memory service available)
...
// ---------------------------------------------------------------------------
// Main distillation entry point — called by cron scheduler or engine
// ---------------------------------------------------------------------------
 ...
 * @param {Object} [config.memoryService] - Memory service instance (for vector integration)
