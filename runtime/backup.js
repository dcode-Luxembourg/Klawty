/** OpenClaw Runtime — Backup Service */
...
function backupAgentFiles(backupDir) {
  ...
  // Auto-discover agent directories (any subdir with AGENT.md, SOUL.md, or MEMORY.md)
  let agentDirs = [];
  try {
...
function backupQdrantSnapshot(backupDir) {
  if (!_qdrantConfig) {
    _logger.info("Vector store backup skipped (not configured)");
  ...
  return new Promise((resolve) => {
    try {
      ...
      const req = transport.request(options, (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode === 200) {
            ...
            _logger.info("Vector store snapshot created", { result: result.result });
            resolve(true);
          } else {
            _logger.warn("Vector store snapshot failed", {
      ...
      req.on("error", (err) => {
        _logger.warn("Vector store snapshot request error", { error: err.message });
        resolve(false);
      });

      req.on("timeout", () => {
        req.destroy();
        _logger.warn("Vector store snapshot timed out");
        resolve(false);
      });

      req.end();
    } catch (err) {
      _logger.warn("Vector store snapshot skipped (config error)", { error: err.message });
      resolve(false);
    }
  });
}
...
async function runBackup() {
  ...
  try {
    ...
    // 4. Vector store snapshot (if configured)
    ...
    // Report
    const summary = `Backup ${date} complete: SQLite=${results.sqlite}, Agents=${results.agentFiles} files, Memory=${results.memoryFiles} files, VectorStore=${results.qdrant}`;
 ...
 *
 * @param {Object} config
 * @param {string} config.basePath - Project root directory
...
function init(config = {}) {
  if (!config.basePath) {
    throw new Error("backup service requires config.basePath (project root directory)");
  }
