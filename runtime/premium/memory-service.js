/** OpenClaw Runtime — Memory Service (Premium — Fleet Tier) */

/**
 * Full 5-tier memory system with Qdrant vector search and relevance scoring.
 *
 * Memory Tiers:
 *   1. Working Memory  — Current task context (injected per cycle)
 *   2. Session Memory   — Today's activity log (/memory/YYYY-MM-DD.md)
 *   3. Agent Memory     — Agent-specific knowledge (/agents/{name}/MEMORY.md)
 *   4. Semantic Memory   — Qdrant vector search (cross-session, long-term)
 *   5. Reference Memory  — Static knowledge bases (/memory/*.md)
 *
 * Context Budget:
 *   60% task content, 25% memory injection, 15% identity/guardrails
 */

"use strict";

const fs = require("fs");
const path = require("path");
const http = require("http");
const https = require("https");
const crypto = require("crypto");

// ---------------------------------------------------------------------------
// Logger — lightweight, replaceable via init()
// ---------------------------------------------------------------------------

let _logger = {
  info: (msg, ctx) => console.log(`[memory-service] ${msg}`, ctx || ""),
  warn: (msg, ctx) => console.warn(`[memory-service] ${msg}`, ctx || ""),
  error: (msg, ctx) => console.error(`[memory-service] ${msg}`, ctx || ""),
  debug: (msg, ctx) => {},
};

// ---------------------------------------------------------------------------
// Configuration — set via init()
// ---------------------------------------------------------------------------

let _basePath = "";
let _agentsDir = "";
let _memoryDir = "";

/** Qdrant configuration */
let _qdrantUrl = process.env.QDRANT_URL || "http://localhost:6333";
let _qdrantApiKey = process.env.QDRANT_API_KEY || "";
let _qdrantCollection = process.env.QDRANT_COLLECTION || "agent_memory";

/** Embedding function — must be provided via init() or set separately */
let _embedFn = null;

// Context window budget (approximate token counts)
const CONTEXT_BUDGET = {
  task: 0.6, // 60% for current task
  memory: 0.25, // 25% for memory injection
  identity: 0.15, // 15% for SOUL/AGENT.md + guardrails
};

// Relevance scoring weights
const SCORE_WEIGHTS = {
  recency: 0.3,
  relevance: 0.5,
  agentSpecificity: 0.2,
};

/**
 * Memory access boundaries — configurable per-agent.
 * Agents with canRead: ['*'] can access all other agents' memories.
 * Agents with readOnly: true cannot write memories.
 * @type {Object<string, {canRead: string[], readOnly?: boolean, note?: string}>}
 */
let _memoryBoundaries = {};

// ---------------------------------------------------------------------------
// Tier 1: Working Memory (Current Task Context)
// ---------------------------------------------------------------------------

function getWorkingMemory(agentName, currentTask) {
  return {
    tier: "working",
    content: currentTask
      ? `## Current Task\n- ID: ${currentTask.id}\n- Title: ${currentTask.title}\n- Status: ${currentTask.status}\n- Priority: ${currentTask.priority}\n- Description: ${currentTask.description || "N/A"}`
      : "## Current Task\nNo active task.",
    score: 1.0, // Always highest priority
  };
}

// ---------------------------------------------------------------------------
// Tier 2: Session Memory (Today's Activity)
// ---------------------------------------------------------------------------

function getSessionMemory() {
  const today = new Date().toISOString().split("T")[0];
  const filePath = path.join(_memoryDir, `${today}.md`);

  try {
    if (fs.existsSync(filePath)) {
      const content = fs.readFileSync(filePath, "utf8");
      return {
        tier: "session",
        content: `## Today's Activity Log\n${content}`,
        score: 0.9,
      };
    }
  } catch (err) {
    _logger.warn("Failed to read session memory", { error: err.message });
  }

  return {
    tier: "session",
    content: "## Today's Activity\nNo activity logged yet today.",
    score: 0.1,
  };
}

// ---------------------------------------------------------------------------
// Tier 3: Agent Memory
// ---------------------------------------------------------------------------

function getAgentMemory(agentName) {
  const filePath = path.join(_agentsDir, agentName, "MEMORY.md");

  try {
    if (fs.existsSync(filePath)) {
      const content = fs.readFileSync(filePath, "utf8");
      return {
        tier: "agent",
        content: `## ${agentName}'s Memory\n${content}`,
        score: 0.8,
      };
    }
  } catch (err) {
    _logger.warn("Failed to read agent memory", { agent: agentName, error: err.message });
  }

  return { tier: "agent", content: `## ${agentName}'s Memory\nNo memory file found.`, score: 0.1 };
}

// ---------------------------------------------------------------------------
// Tier 4: Semantic Memory (Qdrant) — vector search via embeddings
// ---------------------------------------------------------------------------

/**
 * Query Qdrant using real vector similarity search.
 * Embeds the query text via the configured embedding function, then searches the collection.
 */
function queryQdrant(queryText, limit = 5, filter = null) {
  if (!_embedFn) {
    return Promise.reject(
      new Error("Embedding function not configured. Call init() with embedFn."),
    );
  }

  return _embedFn(queryText).then((vector) => {
    return new Promise((resolve, reject) => {
      const url = new URL(`/collections/${_qdrantCollection}/points/query`, _qdrantUrl);

      const body = { query: vector, limit, with_payload: true };
      if (filter) body.filter = filter;

      const payload = JSON.stringify(body);

      const isHttps = url.protocol === "https:";
      const transport = isHttps ? https : http;

      const options = {
        hostname: url.hostname,
        port: url.port || (isHttps ? 443 : 80),
        path: url.pathname,
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "api-key": _qdrantApiKey,
        },
        timeout: 10000,
      };

      const req = transport.request(options, (res) => {
        let data = "";
        res.on("data", (chunk) => (data += chunk));
        res.on("end", () => {
          try {
            const result = JSON.parse(data);
            const points = result.result?.points || result.result || [];
            const memories = (Array.isArray(points) ? points : []).map((point) => ({
              id: point.id,
              score: point.score || 0,
              content: point.payload?.content || point.payload?.text || "",
              agent: point.payload?.agent || "unknown",
              timestamp: point.payload?.timestamp || null,
            }));
            resolve(memories);
          } catch (err) {
            reject(err);
          }
        });
      });

      req.on("error", reject);
      req.on("timeout", () => {
        req.destroy();
        reject(new Error("Qdrant query timeout"));
      });

      req.write(payload);
      req.end();
    });
  });
}

async function getSemanticMemory(agentName, taskContext, limit = 5) {
  try {
    const query = taskContext || `${agentName} recent context`;
    const results = await queryQdrant(query, limit);

    return results.map((mem) => ({
      tier: "semantic",
      content: mem.content,
      score: scoreMemoryChunk(mem, agentName),
      source: `qdrant:${mem.id}`,
    }));
  } catch (err) {
    _logger.warn("Semantic memory query failed", { agent: agentName, error: err.message });
    return [];
  }
}

// ---------------------------------------------------------------------------
// Tier 5: Reference Memory (Static Knowledge Bases)
// ---------------------------------------------------------------------------

function getReferenceMemory(keywords = []) {
  const results = [];

  if (!fs.existsSync(_memoryDir)) return results;

  // Find all .md files in the memory directory that are NOT date-based session logs
  let referenceFiles;
  try {
    referenceFiles = fs
      .readdirSync(_memoryDir)
      .filter((f) => f.endsWith(".md") && !/^\d{4}-\d{2}-\d{2}\.md$/.test(f));
  } catch {
    return results;
  }

  for (const file of referenceFiles) {
    const filePath = path.join(_memoryDir, file);
    try {
      if (fs.existsSync(filePath)) {
        const content = fs.readFileSync(filePath, "utf8");
        // Score based on keyword presence
        const keywordScore =
          keywords.length > 0
            ? keywords.filter((kw) => content.toLowerCase().includes(kw.toLowerCase())).length /
              keywords.length
            : 0.3;

        results.push({
          tier: "reference",
          content: content.substring(0, 2000), // Cap reference content
          score: keywordScore * 0.6, // Reference scores lower than other tiers
          source: file,
        });
      }
    } catch (err) {
      _logger.warn("Failed to read reference file", { file, error: err.message });
    }
  }

  return results;
}

// ---------------------------------------------------------------------------
// Relevance Scoring
// ---------------------------------------------------------------------------

function scoreMemoryChunk(chunk, requestingAgent) {
  let score = 0;

  // Recency score (decay over time)
  if (chunk.timestamp) {
    const ageMs = Date.now() - new Date(chunk.timestamp).getTime();
    const ageHours = ageMs / (1000 * 60 * 60);
    const recencyScore = Math.max(0, 1 - ageHours / (24 * 30)); // Decay over 30 days
    score += recencyScore * SCORE_WEIGHTS.recency;
  } else {
    score += 0.5 * SCORE_WEIGHTS.recency; // Unknown age gets middle score
  }

  // Relevance score (from Qdrant similarity)
  if (chunk.score) {
    score += chunk.score * SCORE_WEIGHTS.relevance;
  }

  // Agent specificity score
  if (chunk.agent === requestingAgent) {
    score += 1.0 * SCORE_WEIGHTS.agentSpecificity;
  } else {
    score += 0.3 * SCORE_WEIGHTS.agentSpecificity;
  }

  return Math.min(1.0, score);
}

// ---------------------------------------------------------------------------
// Context Assembly
// ---------------------------------------------------------------------------

/**
 * Assemble all 5 memory tiers into a single context block.
 * Prioritizes by tier score, caps total at maxTokenBudget.
 *
 * @param {string} agentName - Agent requesting context
 * @param {object} task - Current task object (or null)
 * @param {number} maxTokenBudget - Max tokens for memory block (default 4000)
 * @returns {Promise<string>} - Formatted markdown for prompt injection
 */
async function assembleContext(agentName, task, maxTokenBudget = 4000) {
  const charBudget = maxTokenBudget * 4; // ~1 token ~ 4 chars
  const chunks = [];

  // Tier 1: Working memory (score 1.0)
  if (task) {
    chunks.push({
      tier: "working",
      score: 1.0,
      content: `Current task: "${task.title}" (${task.priority} priority, ${task.status})`,
    });
  }

  // Tier 2: Session memory (score 0.9)
  const session = getSessionMemory();
  if (session.content && session.score > 0.1) chunks.push(session);

  // Tier 3: Agent memory (score 0.8)
  const agentMem = getAgentMemory(agentName);
  if (agentMem.content && agentMem.score > 0.1) {
    chunks.push({ tier: "agent", content: agentMem.content.slice(-2000), score: 0.8 });
  }

  // Tier 4: Semantic memory (score varies)
  if (task?.title) {
    try {
      const results = await recall(task.title, { agent: agentName, limit: 5 });
      if (results?.length > 0) {
        const lines = results
          .map((r) => `  - [${r.agent || "system"}] ${(r.content || "").slice(0, 200)}`)
          .join("\n");
        chunks.push({ tier: "semantic", content: lines, score: 0.7 });
      }
    } catch {
      /* non-blocking */
    }
  }

  // Tier 5: Reference memory (score 0.6)
  if (task) {
    const keywords = (task.title + " " + (task.description || ""))
      .split(/\s+/)
      .filter((w) => w.length > 3);
    const refChunks = getReferenceMemory(keywords);
    chunks.push(...refChunks);
  }

  // Assemble by priority, respecting char budget
  chunks.sort((a, b) => b.score - a.score);
  let usedChars = 0;
  const selected = [];
  for (const chunk of chunks) {
    if (usedChars + chunk.content.length <= charBudget) {
      selected.push(chunk);
      usedChars += chunk.content.length;
    } else if (usedChars < charBudget) {
      const remaining = charBudget - usedChars;
      selected.push({ ...chunk, content: chunk.content.slice(0, remaining) + "\n[...truncated]" });
      usedChars += remaining;
      break;
    }
  }

  _logger.info("Context assembled", {
    agent: agentName,
    totalChunks: chunks.length,
    selectedChunks: selected.length,
    totalChars: usedChars,
    charBudget,
  });

  if (selected.length === 0) return "";

  const TIER_LABELS = {
    working: "CURRENT TASK",
    session: "TODAY'S LOG",
    agent: "YOUR MEMORY",
    semantic: "RELEVANT PAST CONTEXT",
    reference: "REFERENCE DATA",
  };

  const sections = selected.map((s) => {
    return `### ${TIER_LABELS[s.tier] || s.tier}\n${s.content}`;
  });

  return `\n## MEMORY CONTEXT\n${sections.join("\n\n")}`;
}

// ---------------------------------------------------------------------------
// Memory Write Operations
// ---------------------------------------------------------------------------

function appendSessionLog(entry) {
  const today = new Date().toISOString().split("T")[0];
  const filePath = path.join(_memoryDir, `${today}.md`);
  const timestamp = new Date().toISOString().split("T")[1].split(".")[0];
  const line = `- [${timestamp}] ${entry}\n`;

  try {
    // Ensure memory directory exists
    if (!fs.existsSync(_memoryDir)) {
      fs.mkdirSync(_memoryDir, { recursive: true });
    }
    fs.appendFileSync(filePath, line);
  } catch (err) {
    _logger.error("Failed to append session log", { error: err.message });
  }
}

function updateAgentMemory(agentName, section, content) {
  const boundary = _memoryBoundaries[agentName];
  if (boundary && boundary.readOnly) {
    _logger.warn("Attempted write to read-only agent memory", { agent: agentName });
    return false;
  }

  const filePath = path.join(_agentsDir, agentName, "MEMORY.md");

  try {
    let existing = "";
    if (fs.existsSync(filePath)) {
      existing = fs.readFileSync(filePath, "utf8");
    }

    // Append to appropriate section
    const updated = existing + `\n### ${section} — ${new Date().toISOString()}\n${content}\n`;
    fs.writeFileSync(filePath, updated);

    _logger.info("Agent memory updated", { agent: agentName, section });
    return true;
  } catch (err) {
    _logger.error("Failed to update agent memory", { agent: agentName, error: err.message });
    return false;
  }
}

// ---------------------------------------------------------------------------
// Convenience: recall() and recallText()
// ---------------------------------------------------------------------------

/**
 * Semantic recall — returns array of memory objects from Qdrant.
 * Falls back to empty array if Qdrant is unavailable.
 *
 * @param {string} query - Natural language search query
 * @param {object} opts  - { limit?, agent?, minScore? }
 * @returns {Promise<Array>} Memory objects with { content, score, agent, timestamp }
 */
async function recall(query, opts = {}) {
  const { limit = 10, agent = null, minScore = 0 } = opts;
  try {
    const filter = agent ? { must: [{ key: "agent", match: { value: agent } }] } : null;
    const results = await queryQdrant(query, limit, filter);
    return minScore > 0 ? results.filter((r) => r.score >= minScore) : results;
  } catch (err) {
    _logger.warn("recall() failed — Qdrant unavailable", { error: err.message });
    return [];
  }
}

/**
 * Semantic recall as formatted text — for prompt injection.
 *
 * @param {string} query - Natural language search query
 * @param {object} opts  - { limit?, minScore? }
 * @returns {Promise<string|null>} Formatted text block or null if nothing found
 */
async function recallText(query, opts = {}) {
  const results = await recall(query, opts);
  if (!results || results.length === 0) return null;

  const lines = results.map((r) => `- [${r.agent || "unknown"}] ${r.content.slice(0, 300)}`);
  return `## Relevant Memories\n${lines.join("\n")}`;
}

// ---------------------------------------------------------------------------
// Store Memory to Qdrant (embed + upsert)
// ---------------------------------------------------------------------------

/**
 * Store a memory in Qdrant with its vector embedding.
 *
 * @param {string} content - Text content to store
 * @param {object} metadata - { agent, type, source, tags }
 *   type: 'fact' | 'mistake' | 'improvement' | 'insight' | 'migration'
 * @returns {Promise<string>} Point ID
 */
async function storeMemory(content, metadata = {}) {
  if (!_embedFn) {
    throw new Error("Embedding function not configured. Call init() with embedFn.");
  }

  const vector = await _embedFn(content);

  // Deterministic ID based on content hash (prevents exact duplicates)
  const id = crypto.createHash("md5").update(content).digest("hex").slice(0, 16);
  // Qdrant needs unsigned integer or UUID — convert hex to integer
  const numericId = parseInt(id, 16) % Number.MAX_SAFE_INTEGER;

  const point = {
    id: numericId,
    vector,
    payload: {
      content,
      agent: metadata.agent || "system",
      type: metadata.type || "fact",
      source: metadata.source || "unknown",
      tags: metadata.tags || [],
      timestamp: new Date().toISOString(),
    },
  };

  return new Promise((resolve, reject) => {
    const url = new URL(`/collections/${_qdrantCollection}/points`, _qdrantUrl);
    const body = JSON.stringify({ points: [point] });

    const isHttps = url.protocol === "https:";
    const transport = isHttps ? https : http;

    const options = {
      hostname: url.hostname,
      port: url.port || (isHttps ? 443 : 80),
      path: `${url.pathname}?wait=true`,
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "api-key": _qdrantApiKey,
      },
      timeout: 15000,
    };

    const req = transport.request(options, (res) => {
      let data = "";
      res.on("data", (chunk) => (data += chunk));
      res.on("end", () => {
        try {
          const result = JSON.parse(data);
          if (result.status === "ok") {
            _logger.info("Memory stored in Qdrant", {
              agent: metadata.agent,
              type: metadata.type,
              id: numericId,
            });
            resolve(String(numericId));
          } else {
            reject(new Error(`Qdrant upsert failed: ${data.slice(0, 200)}`));
          }
        } catch (err) {
          reject(err);
        }
      });
    });

    req.on("error", reject);
    req.on("timeout", () => {
      req.destroy();
      reject(new Error("Qdrant upsert timeout"));
    });
    req.write(body);
    req.end();
  });
}

/**
 * Store multiple memories in batch (sequential with embedding).
 */
async function storeMemories(items) {
  const results = [];
  for (const item of items) {
    try {
      const id = await storeMemory(item.content, item.metadata);
      results.push({ id, status: "ok" });
    } catch (err) {
      results.push({ id: null, status: "error", error: err.message });
    }
    // Small delay between embeddings to respect rate limits
    await new Promise((r) => setTimeout(r, 120));
  }
  return results;
}

// ---------------------------------------------------------------------------
// High-Level API — learn(), remember(), learnFromTask()
// ---------------------------------------------------------------------------

/**
 * learn() — Store a learning in both Qdrant (semantic) and session log.
 * This is the main write API for all agents.
 *
 * @param {string} agent - Agent name
 * @param {string} content - What was learned
 * @param {object} opts - { type, source, tags, bypassReadOnly }
 * @returns {Promise<{id: string, stored: boolean}>}
 */
async function learn(agent, content, opts = {}) {
  const { type = "fact", source = "runner", tags = [], bypassReadOnly = false } = opts;

  // Boundary check: read-only agents can't store memories (unless bypassed by distiller)
  const boundary = _memoryBoundaries[agent];
  if (boundary && boundary.readOnly && !bypassReadOnly) {
    _logger.warn("Read-only agent attempted to learn", { agent });
    return { id: null, stored: false };
  }

  // 1. Store in Qdrant (semantic long-term)
  let id = null;
  try {
    id = await storeMemory(content, {
      agent,
      type,
      source,
      tags: [...tags, `type:${type}`, `source:${source}`],
    });
  } catch (err) {
    _logger.warn("Failed to store learning in Qdrant", { agent, error: err.message });
  }

  // 2. Append to session log (daily)
  appendSessionLog(`[${agent}] [${type}] ${content.slice(0, 200)}`);

  _logger.info("Learning stored", { agent, type, source, qdrantId: id });
  return { id, stored: !!id };
}

/**
 * remember() — Recall relevant memories for a given context.
 * Combines Qdrant semantic search with agent boundary enforcement.
 *
 * @param {string} agent - Agent requesting memories
 * @param {string} query - Natural language query
 * @param {object} opts - { limit, types, minScore, includeOtherAgents }
 * @returns {Promise<Array>}
 */
async function remember(agent, query, opts = {}) {
  const { limit = 5, types = null, minScore = 0.1, includeOtherAgents = false } = opts;

  // Check access boundaries
  const boundary = _memoryBoundaries[agent];

  // Build Qdrant filter based on boundaries
  let filter = null;
  if (boundary && !includeOtherAgents && !boundary.canRead.includes("*")) {
    // Agent can only read their own memories
    filter = { must: [{ key: "agent", match: { value: agent } }] };
  }

  // Add type filter if specified
  if (types && types.length > 0) {
    const typeFilter = { should: types.map((t) => ({ key: "type", match: { value: t } })) };
    if (filter) {
      filter.must.push(typeFilter);
    } else {
      filter = typeFilter;
    }
  }

  try {
    const results = await queryQdrant(query, limit, filter);
    const filtered = minScore > 0 ? results.filter((r) => r.score >= minScore) : results;

    _logger.info("Memory recall", { agent, query: query.slice(0, 50), results: filtered.length });
    return filtered;
  } catch (err) {
    _logger.warn("remember() failed", { agent, error: err.message });
    return [];
  }
}

/**
 * learnFromTask() — Called after task completion to extract and store learnings.
 *
 * @param {string} agent - Agent that completed the task
 * @param {object} task - { id, title, description, status }
 * @param {string} outcome - What happened
 * @param {object} opts - { reflection }
 */
async function learnFromTask(agent, task, outcome, opts = {}) {
  const { reflection = null } = opts;

  if (!outcome || outcome.length < 20) return;

  // Store the task outcome itself as a fact
  const taskSummary = `Completed task "${task.title}": ${outcome.slice(0, 300)}`;
  await learn(agent, taskSummary, {
    type: "fact",
    source: "task-completion",
    tags: [`task:${task.id || "unknown"}`],
  });

  // If reflection was extracted, store those too
  if (reflection) {
    if (reflection.insight) {
      await learn(agent, reflection.insight, { type: "insight", source: "task-reflection" });
    }
    if (reflection.processImprovement) {
      await learn(agent, reflection.processImprovement, {
        type: "improvement",
        source: "task-reflection",
      });
    }
    if (reflection.mistake) {
      await learn(agent, reflection.mistake, { type: "mistake", source: "task-reflection" });
    }
  }
}

// ---------------------------------------------------------------------------
// getStats() — Return memory system statistics for monitoring
// ---------------------------------------------------------------------------

async function getStats() {
  try {
    const url = new URL(`/collections/${_qdrantCollection}`, _qdrantUrl);
    const isHttps = url.protocol === "https:";
    const transport = isHttps ? https : http;

    return new Promise((resolve) => {
      const req = transport.request(
        {
          hostname: url.hostname,
          port: url.port || (isHttps ? 443 : 80),
          path: url.pathname,
          method: "GET",
          headers: { "api-key": _qdrantApiKey },
          timeout: 5000,
        },
        (res) => {
          let data = "";
          res.on("data", (chunk) => (data += chunk));
          res.on("end", () => {
            try {
              const result = JSON.parse(data);
              const info = result.result || {};
              resolve({
                vectorCount: info.points_count || info.vectors_count || 0,
                segmentCount: info.segments_count || 0,
                status: info.status || "unknown",
                diskSize: info.disk_data_size || 0,
              });
            } catch {
              resolve({ vectorCount: 0, status: "error" });
            }
          });
        },
      );
      req.on("error", () => resolve({ vectorCount: 0, status: "unreachable" }));
      req.on("timeout", () => {
        req.destroy();
        resolve({ vectorCount: 0, status: "timeout" });
      });
      req.end();
    });
  } catch {
    return { vectorCount: 0, status: "error" };
  }
}

// ---------------------------------------------------------------------------
// consolidate() — Deduplicate and prune old/low-quality memories
// ---------------------------------------------------------------------------

/**
 * Deduplicate and prune old/low-quality memories.
 * Run periodically (e.g. weekly) to keep the collection clean.
 *
 * Strategy: scroll through all points for an agent, find near-duplicates
 * (same agent, high text similarity), keep the newest, delete old ones.
 */
async function consolidate(agent, opts = {}) {
  const { dryRun = false, similarityThreshold = 0.95 } = opts;

  _logger.info("Memory consolidation starting", { agent, dryRun });

  // Scroll all points for this agent
  const scrollUrl = new URL(`/collections/${_qdrantCollection}/points/scroll`, _qdrantUrl);
  const isHttps = scrollUrl.protocol === "https:";
  const transport = isHttps ? https : http;

  const points = await new Promise((resolve, reject) => {
    const body = JSON.stringify({
      filter: { must: [{ key: "agent", match: { value: agent } }] },
      limit: 500,
      with_payload: true,
      with_vector: false,
    });

    const req = transport.request(
      {
        hostname: scrollUrl.hostname,
        port: scrollUrl.port || (isHttps ? 443 : 80),
        path: scrollUrl.pathname,
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "api-key": _qdrantApiKey,
        },
        timeout: 15000,
      },
      (res) => {
        let data = "";
        res.on("data", (c) => (data += c));
        res.on("end", () => {
          try {
            const result = JSON.parse(data);
            resolve(result.result?.points || []);
          } catch (e) {
            reject(e);
          }
        });
      },
    );
    req.on("error", reject);
    req.on("timeout", () => {
      req.destroy();
      reject(new Error("Scroll timeout"));
    });
    req.write(body);
    req.end();
  });

  // Find near-duplicates using simple text similarity (character comparison on fingerprints)
  const toDelete = [];
  const seen = new Map(); // content fingerprint → point

  for (const point of points) {
    const content = (point.payload?.content || "").trim().toLowerCase();
    if (!content) continue;

    // Simple fingerprint: first 100 chars normalized
    const fingerprint = content.replace(/\s+/g, " ").slice(0, 100);
    let isDupe = false;

    for (const [existingFp, existingPoint] of seen) {
      // Check if fingerprints are very similar
      const shorter = Math.min(fingerprint.length, existingFp.length);
      const longer = Math.max(fingerprint.length, existingFp.length);
      if (shorter === 0) continue;

      let matches = 0;
      for (let i = 0; i < shorter; i++) {
        if (fingerprint[i] === existingFp[i]) matches++;
      }
      const similarity = matches / longer;

      if (similarity >= similarityThreshold) {
        // Keep the newer one, delete the older
        const existingTs = new Date(existingPoint.payload?.timestamp || 0).getTime();
        const currentTs = new Date(point.payload?.timestamp || 0).getTime();

        if (currentTs >= existingTs) {
          toDelete.push(existingPoint.id);
          seen.set(fingerprint, point);
        } else {
          toDelete.push(point.id);
        }
        isDupe = true;
        break;
      }
    }

    if (!isDupe) {
      seen.set(fingerprint, point);
    }
  }

  _logger.info("Consolidation scan complete", {
    agent,
    totalPoints: points.length,
    duplicates: toDelete.length,
    dryRun,
  });

  if (toDelete.length > 0 && !dryRun) {
    // Delete duplicates
    const delUrl = new URL(`/collections/${_qdrantCollection}/points/delete`, _qdrantUrl);
    await new Promise((resolve, reject) => {
      const body = JSON.stringify({ points: toDelete });
      const req = transport.request(
        {
          hostname: delUrl.hostname,
          port: delUrl.port || (isHttps ? 443 : 80),
          path: `${delUrl.pathname}?wait=true`,
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "api-key": _qdrantApiKey,
          },
          timeout: 15000,
        },
        (res) => {
          let data = "";
          res.on("data", (c) => (data += c));
          res.on("end", () => resolve(data));
        },
      );
      req.on("error", reject);
      req.write(body);
      req.end();
    });

    _logger.info("Duplicates deleted", { agent, count: toDelete.length });
  }

  return {
    totalPoints: points.length,
    duplicatesFound: toDelete.length,
    deleted: dryRun ? 0 : toDelete.length,
  };
}

// ---------------------------------------------------------------------------
// Initialization
// ---------------------------------------------------------------------------

/**
 * Initialize the memory service with workspace configuration.
 *
 * @param {Object} config
 * @param {string} config.basePath - Workspace root directory
 * @param {Function} config.embedFn - Embedding function: async (text) => number[]
 *   Must return a vector (array of floats) for the given text.
 * @param {string} [config.qdrantUrl] - Qdrant server URL (default: env QDRANT_URL or http://localhost:6333)
 * @param {string} [config.qdrantApiKey] - Qdrant API key (default: env QDRANT_API_KEY)
 * @param {string} [config.qdrantCollection] - Qdrant collection name (default: env QDRANT_COLLECTION or 'agent_memory')
 * @param {Object<string, {canRead: string[], readOnly?: boolean}>} [config.memoryBoundaries]
 *   Per-agent access boundaries.
 * @param {Object} [config.logger] - Logger with info/warn/error/debug methods
 */
function init(config = {}) {
  if (!config.basePath) {
    throw new Error("memory-service requires config.basePath (workspace root directory)");
  }

  _basePath = config.basePath;
  _agentsDir = path.join(_basePath, "agents");
  _memoryDir = path.join(_basePath, "memory");

  if (config.embedFn) _embedFn = config.embedFn;
  if (config.qdrantUrl) _qdrantUrl = config.qdrantUrl;
  if (config.qdrantApiKey !== undefined) _qdrantApiKey = config.qdrantApiKey;
  if (config.qdrantCollection) _qdrantCollection = config.qdrantCollection;
  if (config.memoryBoundaries) _memoryBoundaries = config.memoryBoundaries;
  if (config.logger) _logger = config.logger;

  // Ensure directories exist
  if (!fs.existsSync(_memoryDir)) fs.mkdirSync(_memoryDir, { recursive: true });
}

// ---------------------------------------------------------------------------
// Export
// ---------------------------------------------------------------------------

module.exports = {
  init,

  // Tier functions
  assembleContext,
  getWorkingMemory,
  getSessionMemory,
  getAgentMemory,
  getSemanticMemory,
  getReferenceMemory,

  // Low-level Qdrant
  queryQdrant,
  storeMemory,
  storeMemories,

  // High-level API (use these from runners & strategic engine)
  learn,
  remember,
  learnFromTask,
  recall,
  recallText,

  // Write helpers
  appendSessionLog,
  updateAgentMemory,

  // Maintenance
  consolidate,
  getStats,

  // Config
  CONTEXT_BUDGET,
  SCORE_WEIGHTS,
};
