...
This document describes how the Klawty ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`klawty acp` exposes an ACP agent over stdio and forwards prompts to a running
Klawty Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Klawty Gateway
...
- ACP client spawns `klawty acp` and speaks ACP messages over stdio.
...
`klawty acp` resolves the Gateway URL and auth from CLI flags or config:
