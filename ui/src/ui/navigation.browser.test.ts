...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/klawty/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/klawty");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/klawty/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__KLAWTY_CONTROL_UI_BASE_PATH__ = "/klawty";
    const app = mountApp("/klawty/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/klawty");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/klawty/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("klawty.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "klawty.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/klawty" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("klawty.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/klawty",
    });
    expect(JSON.parse(localStorage.getItem("klawty.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/klawty";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/klawty");
