...
describe("provider discovery contract", () => {
  beforeAll(async () => {
    vi.doMock("klawty/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("klawty/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("klawty/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
    ...
    vi.doMock("klawty/plugin-sdk/provider-setup", async () => {
      const actual = await vi.importActual<object>("klawty/plugin-sdk/provider-setup");
      return {
        ...actual,
    ...
    vi.doMock("klawty/plugin-sdk/ollama-setup", async () => {
      const actual = await vi.importActual<object>("klawty/plugin-sdk/ollama-setup");
      return {
        ...actual,
        buildOllamaProvider: (...args: unknown[]) => buildOllamaProviderMock(...args),
      };
