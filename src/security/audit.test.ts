...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: KlawtyConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: KlawtyConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: KlawtyConfig) => string[];
  isConfigured?: (account: unknown, cfg: KlawtyConfig) => boolean;
  isEnabled?: (account: unknown, cfg: KlawtyConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
async function runInstallMetadataAudit(
  ...
  return runSecurityAudit({
    ...
    includeChannelSecurity: false,
    stateDir,
    configPath: path.join(stateDir, "klawty.json"),
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "klawty.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ KLAWTY_STATE_DIR: sharedChannelSecurityStateDir }, () =>
      fn(sharedChannelSecurityStateDir),
    );
  };

  const runSharedExtensionsAudit = async (config: KlawtyConfig) => {
    return runSecurityAudit({
      config,
      includeFilesystem: true,
      includeChannelSecurity: false,
      stateDir: sharedExtensionsStateDir,
      configPath: path.join(sharedExtensionsStateDir, "klawty.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        klawty: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-security-audit-"));
  ...
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    const cases = [
      ...
      },
      {
        ...
        run: async () => {
          const sourceConfig: KlawtyConfig = {
            gateway: {
              bind: "lan",
              auth: {
                token: {
          ...
          const resolvedConfig: KlawtyConfig = {
            gateway: {
              bind: "lan",
              auth: {},
            },
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/klawty-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              ...
              if (args[0] === "inspect" && args.at(-1) === "klawty-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "klawty-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("klawty-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("klawty-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "klawty-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "klawty-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-klawty.json");
  ...
  it("evaluates workspace skill path escape findings", async () => {
    ...
    for (const testCase of cases) {
      ...
      const configPath = path.join(fixture.stateDir, "klawty.json");
      await fs.writeFile(configPath, "{}\n", "utf-8");
      if (!isWindows) {
        await fs.chmod(configPath, 0o600);
      }
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): KlawtyConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    const cases = [
      ...
      },
      {
        ...
        plugin: (sourceConfig: KlawtyConfig) =>
          stubChannelPlugin({
            id: "slack",
            label: "Slack",
            inspectAccount: (cfg) => {
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.klawty/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: KlawtyConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("evaluates hooks ingress auth and routing findings", async () => {
    ...
    } satisfies NonNullable<KlawtyConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        env: {
          KLAWTY_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        },
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies KlawtyConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
        name: "scores unrestricted hooks.allowedAgentIds by remote exposure",
        cfg: { gateway: { bind: "lan" }, hooks: unrestrictedBaseHooks } satisfies KlawtyConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "critical" as const,
      },
      {
        name: "scores hooks request sessionKey override by local exposure",
        cfg: { hooks: requestSessionKeyHooks } satisfies KlawtyConfig,
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.klawty",
      configPath: "/Users/test/Dropbox/.klawty/klawty.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: KlawtyConfig = { logging: { redactSensitive: "off" } };
    ...
    const cases = [
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@klawty/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@klawty/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.KLAWTY_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.KLAWTY_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
      ...
      const cases: Array<{
        ...
        },
        {
          ...
          cfg: {
            gateway: {
              ...
              remote: { url: "wss://remote.example.com:2508", token: "remote-token-xyz789" },
            },
          },
        ...
        },
        {
          ...
          cfg: {
            gateway: {
              mode: "remote",
              auth: { token: "local-token-should-not-use" },
              remote: { url: "wss://remote.example.com:2508", token: "remote-token" },
            },
          },
          env: { token: "env-token" },
        ...
        },
        {
          ...
          cfg: {
            gateway: {
              mode: "remote",
              remote: { url: "wss://remote.example.com:2508", password: "remote-pass" },
            },
          },
