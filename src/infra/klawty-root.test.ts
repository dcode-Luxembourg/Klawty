...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__klawty_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "klawty-root");
...
describe("resolveKlawtyPackageRoot", () => {
  let resolveKlawtyPackageRoot: typeof import("./klawty-root.js").resolveKlawtyPackageRoot;
  let resolveKlawtyPackageRootSync: typeof import("./klawty-root.js").resolveKlawtyPackageRootSync;
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "klawty");
    const pkgRoot = path.join(project, "node_modules", "klawty");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "klawty" }));

    expect(resolveKlawtyPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "klawty");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "klawty.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "klawty" }));

    expect(resolveKlawtyPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveKlawtyPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("falls through from a non-klawty moduleUrl candidate to cwd", async () => {
    const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
    const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
    setFile(path.join(wrongPkgRoot, "package.json"), JSON.stringify({ name: "not-klawty" }));
    setFile(path.join(cwdPkgRoot, "package.json"), JSON.stringify({ name: "klawty" }));
    const moduleUrl = pathToFileURL(path.join(wrongPkgRoot, "dist", "index.js")).toString();

    expect(resolveKlawtyPackageRootSync({ moduleUrl, cwd: cwdPkgRoot })).toBe(cwdPkgRoot);
    await expect(resolveKlawtyPackageRoot({ moduleUrl, cwd: cwdPkgRoot })).resolves.toBe(
      cwdPkgRoot,
    );
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveKlawtyPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveKlawtyPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-klawty package roots", async () => {
    const pkgRoot = fx("not-klawty");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-klawty" }));

    expect(resolveKlawtyPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("falls back from a symlinked argv1 to the node_modules package root", () => {
    const project = fx("symlink-node-modules-fallback");
    const argv1 = path.join(project, "node_modules", ".bin", "klawty");
    state.realpaths.set(abs(argv1), abs(path.join(project, "versions", "current", "klawty.mjs")));
    const pkgRoot = path.join(project, "node_modules", "klawty");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "klawty" }));
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveKlawtyPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveKlawtyPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
