...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: KlawtyConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: KlawtyConfig) {
...
function resolveHeartbeatAgents(cfg: KlawtyConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: KlawtyConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: KlawtyConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: KlawtyConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
