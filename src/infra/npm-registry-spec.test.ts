...
describe("npm registry spec validation", () => {
  it("accepts bare package names, exact versions, and dist-tags", () => {
    expect(validateRegistryNpmSpec("@klawty/voice-call")).toBeNull();
    expect(validateRegistryNpmSpec("@klawty/voice-call@1.2.3")).toBeNull();
    expect(validateRegistryNpmSpec("@klawty/voice-call@1.2.3-beta.4")).toBeNull();
    expect(validateRegistryNpmSpec("@klawty/voice-call@latest")).toBeNull();
    expect(validateRegistryNpmSpec("@klawty/voice-call@beta")).toBeNull();
  });

  it("rejects semver ranges", () => {
    expect(validateRegistryNpmSpec("@klawty/voice-call@^1.2.3")).toContain(
...
describe("npm registry spec parsing helpers", () => {
  it("parses bare, tag, and exact prerelease specs", () => {
    expect(parseRegistryNpmSpec("@klawty/voice-call")).toEqual({
      name: "@klawty/voice-call",
      raw: "@klawty/voice-call",
      selectorKind: "none",
      selectorIsPrerelease: false,
    });
    expect(parseRegistryNpmSpec("@klawty/voice-call@beta")).toEqual({
      name: "@klawty/voice-call",
      raw: "@klawty/voice-call@beta",
      selector: "beta",
      selectorKind: "tag",
      selectorIsPrerelease: false,
    });
    expect(parseRegistryNpmSpec("@klawty/voice-call@1.2.3-beta.1")).toEqual({
      name: "@klawty/voice-call",
      raw: "@klawty/voice-call@1.2.3-beta.1",
...
describe("npm prerelease resolution policy", () => {
  it("blocks prerelease resolutions for bare specs", () => {
    const spec = parseRegistryNpmSpec("@klawty/voice-call");
  ...
  it("blocks prerelease resolutions for latest", () => {
    const spec = parseRegistryNpmSpec("@klawty/voice-call@latest");
    expect(spec).not.toBeNull();
    expect(
      isPrereleaseResolutionAllowed({
        spec: spec!,
  ...
  it("allows prerelease resolutions when the user explicitly opted in", () => {
    const tagSpec = parseRegistryNpmSpec("@klawty/voice-call@beta");
    const versionSpec = parseRegistryNpmSpec("@klawty/voice-call@1.2.3-beta.1");
  ...
  it("allows stable resolutions even for bare and latest specs", () => {
    const bareSpec = parseRegistryNpmSpec("@klawty/voice-call");
    const latestSpec = parseRegistryNpmSpec("@klawty/voice-call@latest");
  ...
  it("formats prerelease resolution guidance based on selector intent", () => {
    ...
    ).toContain(`Use "@klawty/voice-call@beta"`);
    expect(
      formatPrereleaseResolutionError({
        spec: tagSpec!,
        resolvedVersion: "1.2.3-rc.1",
