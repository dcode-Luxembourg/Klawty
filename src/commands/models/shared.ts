...
export async function loadValidConfigOrThrow(): Promise<KlawtyConfig> {
...
export async function updateConfig(
  mutator: (cfg: KlawtyConfig) => KlawtyConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: KlawtyConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: KlawtyConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("klawty agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
