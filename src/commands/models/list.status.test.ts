...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveKlawtyAgentDir: vi.fn().mockReturnValue("/tmp/klawty-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/klawty-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/klawty-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/klawty-dev/klawty.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveKlawtyAgentDir: mocks.resolveKlawtyAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/klawty-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveKlawtyAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/klawty-dev/klawty.json");
    expect(payload.auth.storePath).toBe("/tmp/klawty-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/klawty-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/klawty-agent-custom");
