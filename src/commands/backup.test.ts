...
describe("backup commands", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("klawty-backup-test-");
  ...
  async function withInvalidWorkspaceBackupConfig<T>(fn: (runtime: RuntimeEnv) => Promise<T>) {
    const stateDir = path.join(tempHome.home, ".klawty");
    const configPath = path.join(tempHome.home, "custom-config.json");
    process.env.KLAWTY_CONFIG_PATH = configPath;
    await fs.writeFile(path.join(stateDir, "klawty.json"), JSON.stringify({}), "utf8");
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-workspace-link-"));
    ...
    try {
      ...
      await fs.writeFile(
        path.join(stateDir, "klawty.json"),
        JSON.stringify({
          agents: {
            defaults: {
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    const stateDir = path.join(tempHome.home, ".klawty");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-backups-"));
    try {
      process.env.KLAWTY_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
      ...
      const extractDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-backup-extract-"));
      try {
  ...
  it("optionally verifies the archive after writing it", async () => {
    const stateDir = path.join(tempHome.home, ".klawty");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-backup-verify-on-create-"));
    try {
      await fs.writeFile(path.join(stateDir, "klawty.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".klawty");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-backup-link-"));
  ...
  it("falls back to the home directory when cwd is a symlink into a backed-up source tree", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".klawty");
    const workspaceDir = path.join(stateDir, "workspace");
    const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-backup-cwd-link-"));
