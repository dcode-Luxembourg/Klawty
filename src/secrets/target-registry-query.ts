...
const KLAWTY_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
  (entry) => entry.configFile === "klawty.json",
);
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of KLAWTY_COMPILED_SECRET_TARGETS) {
...
const KLAWTY_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: KLAWTY_COMPILED_SECRET_TARGETS,
    entriesById: KLAWTY_TARGETS_BY_ID,
  });
