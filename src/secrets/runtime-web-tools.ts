...
type FetchConfig = NonNullable<KlawtyConfig["tools"]>["web"] extends infer Web
...
function setResolvedFirecrawlApiKey(params: { resolvedConfig: KlawtyConfig; value: string }): void {
  const tools = ensureObject(params.resolvedConfig as Record<string, unknown>, "tools");
  const web = ensureObject(tools, "web");
