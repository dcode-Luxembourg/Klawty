...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.KLAWTY_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.KLAWTY_DISABLE_LAZY_SUBCOMMANDS);
};

export const loadValidatedConfigForPluginRegistration = async (): Promise<KlawtyConfig | null> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live Klawty docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  {
    name: "update",
    description: "Update Klawty and inspect update channel status",
    hasSubcommands: true,
    register: async (program) => {
