...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `klawty-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.KLAWTY_CONFIG_PATH = path.join(root, "klawty.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "klawty.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "klawty-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: KlawtyConfig) => composeTestConfig(cfg as Record<string, unknown>),
    loadConfig: () => {
      const configPath = resolveConfigPath();
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadKlawtyPlugins: () => pluginRegistryState.registry,
  };
});
