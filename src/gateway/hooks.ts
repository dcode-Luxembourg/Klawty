...
export function resolveHooksConfig(cfg: KlawtyConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: KlawtyConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-klawty-token"] === "string" ? req.headers["x-klawty-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-klawty-idempotency-key"]) ||
