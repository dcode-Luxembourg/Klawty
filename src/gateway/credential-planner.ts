...
type GatewaySecretDefaults = NonNullable<KlawtyConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${KLAWTY_GATEWAY_TOKEN}` from being
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["KLAWTY_GATEWAY_PASSWORD", "CLAWDBOT_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}
