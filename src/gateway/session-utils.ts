...
import { type KlawtyConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: KlawtyConfig): string[] {
...
export function listAgentsForGateway(cfg: KlawtyConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: KlawtyConfig): string {
...
function resolveSessionStoreAgentId(cfg: KlawtyConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: KlawtyConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: KlawtyConfig): GatewaySessionsDefaults {
