...
function collectConfigEnvVarsByTarget(cfg?: KlawtyConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: KlawtyConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: KlawtyConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: KlawtyConfig): Record<string, string> {
...
export function applyConfigEnvVars(cfg: KlawtyConfig, env: NodeJS.ProcessEnv = process.env): void {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.KLAWTY_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
