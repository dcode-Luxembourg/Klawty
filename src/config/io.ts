...
import type { KlawtyConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareKlawtyVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  klawty config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  klawty config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: KlawtyConfig,
  pathSegments: string[],
): { changed: boolean; next: KlawtyConfig } {
...
function coerceConfig(value: unknown): KlawtyConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as KlawtyConfig;
}
...
function stampConfigVersion(cfg: KlawtyConfig): KlawtyConfig {
...
function warnIfConfigFromFuture(cfg: KlawtyConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareKlawtyVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Klawty (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as KlawtyConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): KlawtyConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as KlawtyConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: KlawtyConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as KlawtyConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.KLAWTY_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.KLAWTY_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.KLAWTY_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.KLAWTY_WATCH_SESSION === "string" &&
        deps.env.KLAWTY_WATCH_SESSION.trim().length > 0
          ? deps.env.KLAWTY_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.KLAWTY_WATCH_COMMAND === "string" &&
        deps.env.KLAWTY_WATCH_COMMAND.trim().length > 0
          ? deps.env.KLAWTY_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `KLAWTY_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: KlawtyConfig | null = null;
let runtimeConfigSourceSnapshot: KlawtyConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.KLAWTY_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.KLAWTY_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): KlawtyConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): KlawtyConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: KlawtyConfig;
  candidate: KlawtyConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: KlawtyConfig): KlawtyConfig {
...
export function loadConfig(): KlawtyConfig {
...
export async function readBestEffortConfig(): Promise<KlawtyConfig> {
...
export async function writeConfigFile(
  ...
  // edits to klawty.json remain visible (no stale snapshot).
}
