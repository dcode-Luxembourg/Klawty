...
function telegramCfg(): KlawtyConfig {
  return { channels: { telegram: { botToken: "tok" } } } as KlawtyConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: KlawtyConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as KlawtyConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as KlawtyConfig;
...
function createSignalAccountOverrideCfg(): KlawtyConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("derives discord action listings from channel and moderation gates", () => {
    const cases = [
      {
        name: "defaults",
        cfg: { channels: { discord: { token: "d0" } } } as KlawtyConfig,
        expectUploads: true,
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as KlawtyConfig,
        accountId: undefined,
        params: {
