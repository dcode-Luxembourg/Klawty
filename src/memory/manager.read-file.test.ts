...
function createMemorySearchCfg(options: { workspaceDir: string; indexPath: string }): KlawtyConfig {
  return {
    agents: {
      defaults: {
...
describe("MemoryIndexManager.readFile", () => {
  ...
  beforeEach(async () => {
    resetEmbeddingMocks();
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-mem-read-"));
