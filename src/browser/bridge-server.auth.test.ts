...
function buildResolvedConfig(): ResolvedBrowserConfig {
  return {
    ...
    extraArgs: [],
    color: DEFAULT_KLAWTY_BROWSER_COLOR,
    executablePath: undefined,
    headless: true,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_KLAWTY_BROWSER_PROFILE_NAME,
    profiles: {
...
describe("startBrowserBridgeServer auth", () => {
  ...
  it("accepts x-klawty-password when authPassword is set", async () => {
    await expectAuthFlow(
      { authPassword: "secret-password" },
      { "x-klawty-password": "secret-password" },
    );
  });
