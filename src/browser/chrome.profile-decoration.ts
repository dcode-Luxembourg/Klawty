...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".klawty-profile-decorated");
}
 ...
 */
export function decorateKlawtyProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_KLAWTY_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_KLAWTY_BROWSER_COLOR).toUpperCase();
