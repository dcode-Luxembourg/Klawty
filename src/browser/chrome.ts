...
export function resolveKlawtyUserDataDir(profileName = DEFAULT_KLAWTY_BROWSER_PROFILE_NAME) {
...
export async function launchKlawtyChrome(
  ...
  const userDataDir = resolveKlawtyUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_KLAWTY_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateKlawtyProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 klawty browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`klawty browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`klawty browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 klawty browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
