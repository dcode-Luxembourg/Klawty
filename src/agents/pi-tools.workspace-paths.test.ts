...
import { createKlawtyCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("klawty-ws-", async (workspaceDir) => {
      await withTempDir("klawty-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createKlawtyCodingTools({ workspaceDir });
          ...
          expect(await fs.readFile(path.join(workspaceDir, editFile), "utf8")).toBe("hello klawty");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("klawty-ws-", async (workspaceDir) => {
      await withTempDir("klawty-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("klawty-ws-", async (workspaceDir) => {
      const cfg: KlawtyConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createKlawtyCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-klawty.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("klawty-sandbox-", async (sandboxDir) => {
      await withTempDir("klawty-workspace-", async (workspaceDir) => {
        ...
        const tools = createKlawtyCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
