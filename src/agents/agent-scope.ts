...
type AgentEntry = NonNullable<NonNullable<KlawtyConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: KlawtyConfig): AgentEntry[] {
...
export function listAgentIds(cfg: KlawtyConfig): string[] {
...
export function resolveDefaultAgentId(cfg: KlawtyConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: KlawtyConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: KlawtyConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: KlawtyConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
