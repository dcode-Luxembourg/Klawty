...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "klawty" }),
                      },
                    },
                  ],
                },
    ...
    const result = await tool?.execute?.("call-1", { query: "latest klawty release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://klawty.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://klawty.ai/docs"]);
