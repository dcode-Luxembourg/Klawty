 ...
 * `klawty update`. Use it to verify the completion installation flow
...
\`klawty update\`, \`klawty doctor\`, and \`klawty onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Klawty Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`klawty completion --install\` later to enable.`));
    return;
  }
