...
type DriverMode = "token" | "webhook" | "klawty";
...
function resolveStateDir(): string {
  const override = process.env.KLAWTY_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.KLAWTY_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".klawty");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver klawty] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Klawty spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|klawty> Driver transport mode (default: token)\n" +
    ...
    "  --klawty-bin <path>        Klawty CLI binary for driver=klawty (default: klawty)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  KLAWTY_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  KLAWTY_DISCORD_SMOKE_DRIVER\n" +
    "  KLAWTY_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  KLAWTY_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  KLAWTY_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  KLAWTY_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  KLAWTY_DISCORD_SMOKE_AGENT\n" +
    "  KLAWTY_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  KLAWTY_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  KLAWTY_DISCORD_SMOKE_POLL_MS\n" +
    "  KLAWTY_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  KLAWTY_DISCORD_SMOKE_KLAWTY_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.KLAWTY_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.KLAWTY_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.KLAWTY_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.KLAWTY_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.KLAWTY_DISCORD_SMOKE_BOT_TOKEN ||
  ...
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.KLAWTY_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.KLAWTY_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.KLAWTY_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.KLAWTY_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.KLAWTY_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const klawtyBin =
    resolveArg("--klawty-bin") || process.env.KLAWTY_DISCORD_SMOKE_KLAWTY_BIN || "klawty";
...
async function klawtyCliJson<T>(params: { klawtyBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.klawtyBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`klawty ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithKlawty(params: {
  klawtyBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await klawtyCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    klawtyBin: params.klawtyBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "klawty") {
    return await readMessagesWithKlawty({
      klawtyBin: params.args.klawtyBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `klawty-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await klawtyCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("klawty message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "klawty"
            ? await readMessagesWithKlawty({
                klawtyBin: args.klawtyBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Klawty.`,
        diagnostics: {
          bindingCandidates: [
            {
