...
const EXPECTED_REPOSITORY_URL = "https://github.com/klawty/klawty";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "klawty") {
    errors.push(`package.json name must be "klawty"; found "${pkg.name ?? ""}".`);
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`klawty-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `klawty-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
