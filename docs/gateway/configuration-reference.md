...
description: "Complete field-by-field reference for ~/.klawty/klawty.json"
summary: "Complete reference for every Klawty config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.klawty/klawty.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Klawty uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.klawty/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `klawty doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `klawty doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["klawty-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-klawty",
          requireMention: false,
...
- Klawty additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `klawty plugins install @klawty/mattermost`.
...
- `commands.callbackUrl` must resolve to the Klawty gateway endpoint and be reachable from the Mattermost server.
...
Klawty spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `klawty channels add` (or channel onboarding) while still on a single-account top-level channel config, Klawty moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `klawty doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@klawty", "klawty"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@klawty"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `klawty.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Klawty auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/klawty" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Klawty assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Klawty aborts it. Default: `900`.
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          apparmorProfile: "klawty-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "klawty-sandbox-browser:bookworm-slim",
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Klawty materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.klawty/sandboxes`
...
In `remote` mode, host-local edits made outside Klawty are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `klawty.json`.
noVNC observer access uses VNC auth by default and Klawty emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `klawty-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from KLAWTY_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `KLAWTY_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `KLAWTY_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `KLAWTY_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.klawty/workspace",
        agentDir: "~/.klawty/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@klawty"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.klawty/workspace-home" },
      { id: "work", workspace: "~/.klawty/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Klawty resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.klawty/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.klawty/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.klawty/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.klawty/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Klawty starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.klawty/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Klawty treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
- Files are materialized into the child workspace at `.klawty/attachments/<uuid>/` with a `.manifest.json`.
...
Klawty uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.klawty/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `KLAWTY_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Klawty forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `klawty onboard --auth-choice opencode-zen` or `klawty onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `klawty onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `klawty onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `klawty onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `klawty onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `klawty onboard --auth-choice minimax-api`.
...
- Loaded from `~/.klawty/extensions`, `<workspace>/.klawty/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Klawty plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Klawty plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Klawty applies those as sanitized agent settings, not as raw Klawty config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `klawty plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or KLAWTY_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `KLAWTY_GATEWAY_PORT` > `gateway.port` > `2508`.
- `bind`: `auto`, `loopback` (default), `lan` (`0.0.0.0`), `tailnet` (Tailscale IP only), or `custom`.
- **Legacy bind aliases**: use bind mode values in `gateway.bind` (`auto`, `loopback`, `lan`, `tailnet`, `custom`), not host aliases (`0.0.0.0`, `127.0.0.1`, `localhost`, `::`, `::1`).
- **Docker note**: the default `loopback` bind listens on `127.0.0.1` inside the container. With Docker bridge networking (`-p 2508:2508`), traffic arrives on `eth0`, so the gateway is unreachable. Use `--network host`, or set `bind: "lan"` (or `bind: "custom"` with `customBindHost: "0.0.0.0"`) to listen on all interfaces.
...
- `KLAWTY_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `KLAWTY_APNS_RELAY_BASE_URL` / `KLAWTY_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `KLAWTY_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
KLAWTY_CONFIG_PATH=~/.klawty/a.json \
KLAWTY_STATE_DIR=~/.klawty-a \
klawty gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.klawty-dev` + port `19001`), `--profile <name>` (uses `~/.klawty-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.klawty/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-klawty-token: <token>`.
...
```json5
{
  hooks: {
    gmail: {
      account: "klawty@gmail.com",
      topic: "projects/<project-id>/topics/gog-gmail-watch",
      subscription: "gog-gmail-watch-push",
      pushToken: "shared-push-token",
      hookUrl: "http://127.0.0.1:2508/hooks/gmail",
      includeBody: true,
      maxBytes: 20000,
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `KLAWTY_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.klawty/workspace/canvas",
    liveReload: true,
    // enabled: false, // or KLAWTY_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__klawty__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__klawty__/a2ui/`
...
- Also serves A2UI at `/__klawty__/a2ui/`.
...
- Hostname defaults to `klawty`. Override with `KLAWTY_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.klawty/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `klawty dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.klawty/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${KLAWTY_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `klawty.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.klawty/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/klawty-vault-resolver",
...
- Legacy OAuth imports from `~/.klawty/credentials/oauth.json`.
...
- Default log file: `/tmp/klawty/klawty-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Klawty.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `KLAWTY_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `klawty doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `klawty.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
