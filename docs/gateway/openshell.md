---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Klawty agents"
read_when:
...
OpenShell is a managed sandbox backend for Klawty. Instead of running Docker
containers locally, Klawty delegates sandbox lifecycle to the `openshell` CLI,
...
- Klawty Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Klawty creates an OpenShell
...
- Before `exec`, Klawty syncs the local workspace into the OpenShell sandbox.
- After `exec`, Klawty syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace
  remains the source of truth between turns.

Best for:

- You edit files locally outside Klawty and want those changes visible in the
...
- When the sandbox is first created, Klawty seeds the remote workspace from
...
- Klawty does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Klawty after the initial seed,
the remote sandbox does **not** see those changes. Use
`klawty sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"klawty"`    | Sandbox source for first-time create                  |
...
1. Klawty calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Klawty calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `klawty sandbox` commands
