...
summary: Klawty.app 连接远程 Gateway 网关的 SSH 隧道设置
...
# 使用远程 Gateway 网关运行 Klawty.app

Klawty.app 使用 SSH 隧道连接到远程 Gateway 网关。本指南向你展示如何设置。
...
│                        Client Machine                          │
│                                                              │
│  Klawty.app ──► ws://127.0.0.1:2508 (local port)           │
│                     │                                        │
│                     ▼                                        │
│  SSH Tunnel ────────────────────────────────────────────────│
│                     │                                        │
...
│                         Remote Machine                        │
│                                                              │
│  Gateway WebSocket ──► ws://127.0.0.1:2508 ──►              │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
旧版注意事项：如果存在任何遗留的 `com.klawty.ssh-tunnel` LaunchAgent，请将其删除。
...
| 组件                               | 功能                                |
| ---------------------------------- | ----------------------------------- |
| `LocalForward 2508 127.0.0.1:2508` | 将本地端口 2508 转发到远程端口 2508 |
| `ssh -N`                           | SSH 不执行远程命令（仅端口转发）    |
| `KeepAlive`                        | 隧道崩溃时自动重启                  |
| `RunAtLoad`                        | 代理加载时启动隧道                  |

Klawty.app 连接到你的客户端机器上的 `ws://127.0.0.1:2508`。SSH 隧道将该连接转发到运行 Gateway 网关的远程机器的端口 2508。
