...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Klawty to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Klawty releases, so no separate plugin install
is required.
...
```bash
klawty plugins install @klawty/feishu
```
...
If you just installed Klawty, run onboarding:
...
2. Configuring app credentials in Klawty
...
1. You already ran `klawty channels add` for Feishu
2. The gateway is running (`klawty gateway status`)
...
```bash
klawty pairing approve feishu <CODE>
```
...
2. Run `klawty logs --follow` and look for `chat_id`
...
2. Run `klawty logs --follow` and look for `open_id`
...
| `klawty gateway restart` | Restart gateway service       |
| `klawty logs --follow`   | Tail gateway logs             |
...
4. Check logs: `klawty logs --follow`
...
5. Ensure the gateway is running: `klawty gateway status`
6. Check logs: `klawty logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.klawty/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.klawty/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
